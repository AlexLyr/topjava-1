...
public class JdbcUserRepositoryImpl implements UserRepository {
    ...
    public User get(int id) {
        List<User> users = jdbcTemplate.query("SELECT * FROM users, user_roles WHERE id=user_roles.user_id AND id=?", USER_EXTRACTOR, id);
...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public void insertBatch(String sql,User user){
        ...
        jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
            ...
            /*while(!roles.isEmpty()) {
                jdbcTemplate.update(sql, newKey.intValue(), roles.remove(0));
            }
            roles=null;*/

                    /*(connection -> {
                PreparedStatement ps=connection.prepareStatement("INSERT INTO user_roles(user_id, role) VALUES (?,?)");
                ps.setInt(1,newKey.intValue());
                ps.setArray(2,connection.createArrayOf("varchar",roles.toArray(new String[roles.size()])));
                return ps;
            });*/
    }
